...
Status: IN PROGRESS — Phase 1, 2, 3, and 4 shipped (Track A scaffold + Track B grep guard + Track E Phase 1 signed DTLS fp passthrough). Track C (Sender Keys for rooms) REVERTED to legacy per-room AES on 2026-05-20 — see note below.
...
| C — Rooms (Sender Keys) | **REVERTED 2026-05-20** — channel send/edit now always uses legacy per-room AES-256-GCM (`State.roomKeys[room]`). Reasons: (1) libsignal sender chain cannot decrypt sender's own messages so own bubbles needed an ever-growing plaintext-cache stack and still flickered to 🔒; (2) "iteration already consumed" replay protection tripped on every history reload / federation echo / WS retransmit; (3) offline users miss the SKDM and can never recover those past messages; (4) Telegram / Discord bridge bots have no Signal identity so bridge channels were silently broken (inbound text shown as ciphertext, outbound bridge worked only via separate plaintext flag). Implementation: `messages.js` send + edit drop the `Signal.room.encryptMessage` branch and always call `Crypto.encrypt(text, key)` (or pass plaintext when no room key is set). `ws.js` `decryptMsg` keeps a cache-only read path for existing v2 envelopes sitting in channel history — sticky plaintext cache hit renders; cache miss leaves 🔒 placeholder permanently (no libsignal replay attempt). DMs / calls / wall / linked devices stay on Signal. SKDM relay backend + `signal_room.js` + plaintext caches in `messages.js` are left dormant (not exercised but not deleted, in case we revisit with `@signalapp/libsignal-client` WASM and a Signal-aware bridge worker). | ⛔ reverted | (this commit) |
| D — Wall posts + media at rest | phase 1+2: per-post AES-256-GCM payload key + per-recipient `Signal.encryptDM` wrap stored in `wall_post_keys`; `wall_posts.enc_v`/`audience`/`ciphertext` columns; `POST /api/wall/posts/encrypted` (60/h rate-limit, 4 MiB ct cap, 5000-recipient cap, 4096 B wrap cap); `GET /api/wall/audience-recipients`; `static/js/wall_crypto.js` (`publish` / `decryptList` / `decryptInline` / `rememberAuthorKey`, sessionStorage key cache); composer flips encrypted branch for friends/followers/custom-list audiences; feed/profile/reposts/single-post all decrypt inline; `social.post.created.encrypted` federation event carries `ciphertext_b64` + per-recipient `wrapped_keys`; encrypted body covers `content` + `track_*` only — media stays plaintext on server (`media_data` column unchanged) per user 2026-05-17 decision so profile media/music/reels tabs keep working; phase 3 (encrypted media at rest) deferred | ✅ shipped | `68e7826` |
| D — Wall posts (audience-gating audit) | follow-up: `wall_post_viewer_in_audience(post_id, viewer_id)` helper checks `wall_post_keys` membership for enc_v=2 rows (always-true for enc_v=0); POST `/api/wall/posts/{id}/reactions` + POST `/api/wall/posts/{id}/comments` + GET `/api/wall/posts/{id}/comments` return 403 "Not in audience" for non-audience viewers; `get_reels_posts` friends branch and `get_personalized_reels` both `LEFT JOIN wall_post_keys` and gate with `enc_v=0 OR user_id=viewer OR wpk.wrapped_key IS NOT NULL`; `/api/social/posts/{id}/media` now runs the same audience check for enc_v=2 (custom-list audiences can be stricter than the `privacy` column suggests); explore feed unaffected (already filters `privacy='public'` which excludes all enc_v=2 rows) | ✅ shipped | (this commit) |
...
| F — Linked devices | phase 2+3: QR-based pairing — `signal_pairing_tokens` table (32-byte token / hex, 5-min TTL, status `pending`→`claimed`→`complete`); helpers `signal_pairing_create`/`_claim`/`_lookup`/`_approve`/`_status`; endpoints `POST /api/signal/devices/pairing/start`, `POST /{token}/claim`, `GET /{token}`, `POST /{token}/approve`, `GET /{token}/status` (status is unauth — token = capability); generic `Signal.getMyIdentityPubB64` + `Signal.signWithIdentity(bytes)` XEdDSA helpers; vendored `qrcode-generator@1.4.4` + `jsQR@1.4.0`; new `static/js/devices.js` (`Devices.openManager/startPairing/scanPairing/approvePairing/revoke/registerThisDevice`); new `modal-linked-devices`/`modal-link-device`/`modal-scan-device` modals + Settings → Security entry point; QR payload `{v:1,type:'frogtalk-link',token,server}`; mobile 44 px touch targets + theme tokens throughout; cross-device fanout-encrypt deferred to phase 4 | ✅ shipped | — | — |
| G — Sealed Sender + metadata | DEFERRED (2026-05-17) — vendored `libsignal-protocol-typescript@0.0.16` ships no sealed-sender helper, so a real implementation needs either a WASM library swap (large surface — touches every existing Signal store path) or a homegrown sealed envelope built on the existing X3DH identity keys. Skipped per user direction to keep the app shipping. Mitigations already in place that approximate the threat model: per-DM Signal envelopes mean the wire payload carries no plaintext; tor node 161.97.182.73 is reachable over the hidden service so the network-level recipient is concealed by Tor for those clients; DM rows do still carry `sender_id`/`recipient_id` in the DB and are protected only by FastAPI auth + sqlite RBAC — anyone with server access can correlate metadata. Revisit when we adopt `@signalapp/libsignal-client` WASM | ⛔ deferred | — |
...
| H — Cleanup (transitional code) | backend + frontend code purge: removed `FROGTALK_DM_ENC_V2` / `FROGTALK_ROOM_ENC_V2` flag gating from `routers/signal.py`; deleted `GET/POST /api/users/{user_id}/pubkey` v1 ECDH endpoints + `set_ecdh_pub_key` helper + schema-migration ALTER (tombstoned as comment, column left orphaned per SQLite limits); deleted `_v2EnvelopeUsed` / `_editV2Used` v1 fallback branches in `static/js/dms.js` (send + edit now hard-fail when Signal isn't ready instead of leaking plaintext); deleted v1 ECDH shared-key derivation + pubkey-fetch fallback in `_decryptDMPreviewContent` + DM open path (`_getDMSharedKey` is now a stub); deleted `deriveSharedSecret` / `encryptMsg` / `decryptMsg` from `state.js`; deleted `/api/users/pubkey` publish from `app.js` post-login flow; rewired `resetEncryptionKeys` in `ui.js` to `Signal.resetIdentity` (new public surface that wipes the Signal store + republishes a fresh bundle); messages.js room send/edit now requires Sender-Keys for non-bridge rooms (bridge-outbound rooms keep transitional AES so the bridge can still forward); DM media in flight stays under `Crypto.encryptPayload` until Track D | ✅ shipped | `5300158` |
