...
Default admin username is set in `board_config.php` (`ADMIN_USER`). Change `ADMIN_PASS_HASH` before any public deployment:
...
nginx routes `/board`, `/board_uploads`, and `/board_previews` to this tree (see `deploy/nginx.conf`). Deploy board files with the rest of the repo; restart `frogtalk` (Python) after app changes. PHP files are picked up on the next request.
...
- IP bans (real client IP when behind Cloudflare / `X-Forwarded-For`)
- Per-board live chat
...
- Do not commit private deploy scripts or server hostnames to the public repo.
