...
**Updates:** deploy board PHP with the rest of the node tree via `deploy.sh` or `install.sh update-apply`. Restart `frogtalk` after Python changes; PHP is picked up on the next request.
