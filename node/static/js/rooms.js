...
const Rooms = (() => {
  ...
  function isSwitchOverlayForRoom(room) {
    const o = document.getElementById(_CHAT_TRANSITION_ID);
    if (!o) return true;
    const key = String(o.dataset.ftSwitchKey || '').trim().toLowerCase();
  ...
  function _shellHasContent(room) {
    ...
    if (!shell || !shell.children.length) return false;
    return _shellHasRealMessages(shell);
  }

  function _shellHasRealMessages(shellIn) {
    const shell = shellIn || _chatShell();
  ...
  function _channelSwitchUiPending() {
    try {
      if (window.FtCompose?.isChannelLoading?.()) return true;
    } catch {}
    try {
  ...
  function showChatTransition(name, type, dmPeer, phase, roomOpts) {
    ...
    if (opts.switchToken != null && !_isSwitchAlive(opts.switchToken)) return;
    ...
    if (existing && switchKey && existing.dataset.ftSwitchKey === switchKey) {
      ...
      if (opts.switchToken != null) existing.dataset.ftSwitchSeq = String(opts.switchToken);
  ...
  function isSwitchOverlayVisible() {
    const area = document.getElementById('messages-area');
    return !!(document.getElementById(_CHAT_TRANSITION_ID)
      || (area && area.classList.contains('chat-switching')));
  }

  function _scrubStaleSwitchChrome(area) {
    const mount = area || document.getElementById('messages-area');
    if (!mount) return;
    try {
      mount.querySelectorAll('.chat-transition-overlay').forEach((el) => {
        if (el.id === _CHAT_TRANSITION_ID) return;
        el.remove();
      });
    } catch {}
  ...
  function _disarmSwitchWatchdog() {
    if (_switchWatchdogTimer) {
      clearTimeout(_switchWatchdogTimer);
      _switchWatchdogTimer = null;
    }
  }

  function _armSwitchWatchdog(switchToken, roomName) {
    _disarmSwitchWatchdog();
    const expectRoom = String(roomName || '').trim().toLowerCase();
    if (!expectRoom) return;
    _switchWatchdogTimer = setTimeout(() => {
      _switchWatchdogTimer = null;
  ...
  function finishChannelSwitch(room, opts) {
    const options = opts && typeof opts === 'object' ? opts : {};
    const r = String(room || State?.currentRoom || '').trim().toLowerCase();
    if (options.onlyIfCurrent !== false && r) {
      if (String(State?.currentRoom || '').trim().toLowerCase() !== r) return;
    }
  ...
  function clearChatTransition(opts) {
    ...
    if (overlay.dataset.ftSwitchSeq && Number(overlay.dataset.ftSwitchSeq) !== _switchSeq && !force) {
      return;
    }
  ...
  async function _handleCwDecline(name, opts = {}) {
    ...
    clearChatTransition({ finish: false, force: true });
  ...
  async function saveChannelSettings() {
    ...
    const dirListed = (_currentRoomData?.room?.type === 'private')
      ? false
      : !!document.getElementById('ch-dir-listed')?.checked;
  ...
  function showChannelAbout(roomName = null) {
    ...
    if (aboutAccess) {
      if (room.type === 'private') {
        aboutAccess.textContent = 'Private (secret + invite)';
      } else if (room.is_public) {
        aboutAccess.textContent = 'Listed in directory';
      } else {
        aboutAccess.textContent = 'Unlisted — join by name or invite';
      }
    }
