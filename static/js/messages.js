...
const Messages = (() => {
  ...
  function _authorDisplay(msg) {
    // Bridged messages are attributed to the bridge owner user_id in DB
  ...
  function _authorHandleHtml(msg) {
    if (!msg || !msg.display_name) return '';
    if (msg.display_name === msg.nickname) return '';
    if (msg.bridge_platform && msg.display_name === msg.bridge_source_name) return '';
  ...
  function _formatContent(text, opts) {
    ...
    // Extract URLs to placeholders BEFORE the @mention / #channel passes.
    // Otherwise URLs that legitimately contain "@" (TikTok profile links,
    // youtube.com/@channel, etc.) would have their path turned into a
    // mention pill, breaking both the link text and the href.
    const urlSlots = [];
...
function _buildMsgHtml(msg, isCont) {
  ...
  const authorName = msg.bridge_platform
    ...
    : (msg.display_name || msg.nickname);
  const handleHtml = (msg.display_name && msg.display_name !== msg.nickname && (!msg.bridge_platform || msg.display_name !== msg.bridge_source_name))
    ? `<span class="msg-author-handle">@${UI.escHtml(msg.nickname)}</span>`
    : '';
  return `<div class="msg-group" id="msg-${msg.id}">
    ...
    <div class="msg-body">
      <div class="msg-meta">
        <span class="msg-author">${UI.escHtml(authorName)}</span>${handleHtml}
        <span class="msg-time">${time}</span>
      </div>
